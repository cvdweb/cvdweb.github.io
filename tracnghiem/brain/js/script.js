...
function chkUser() {
	...
	if (tempName == null || tempName == '' || tempName == 'undefined') {
		...
		if (
			...
			errH.innerHTML = tempName + ', Nhập lại mật khẩu của bạn.'; // display this text
...
function askPass() {
	...
	errH.innerHTML = tempName + ', Nhập mật khẩu của bạn'; // display this line with user's name
}
...
function chkPass(btn) {
	...
	if (pass.value == 11223344) {
		...
		setTimeout(function() {
			...
			err.innerHTML = 'Mật khẩu đúng rồi!'; // in 2nd line
			err.style.color = '#28a745';

			btn.innerHTML = 'Bắt đầu bài trắc nghiệm &gt;'; // replace text of input box to this
		...
		setTimeout(function() {
			...
			err.style.color = '#dc3545';
			err.innerHTML = 'Mật khẩu chưa đúng!';
...
function topping(n) {
	...
	if (n == totQ.length - 1)
		document.getElementById('next-button').innerHTML = 'Xác nhận';
	else if (n == totQ.length) {
		document.getElementById('next-button').innerHTML = 'Không có câu hỏi';
		nextBtn.setAttribute('disabled', 'disabled');
	} else document.getElementById('next-button').innerHTML = 'Tiếp';
...
function showResult(percentage, ca) {
	if (percentage == 100) {
		RColor = 'teal';
		resultFb.innerHTML = 'Wohoo.. Bạn thật tuyệt vời, xuất sắc!';
		correctAns.innerHTML = 'Số câu đúng: ' + ca;
	} else if (percentage >= 80) {
		RColor = 'green';
		resultFb.innerHTML = 'Làm rất tốt! giỏi.';
		correctAns.innerHTML = 'Số câu đúng: ' + ca;
	} else if (percentage >= 65) {
		RColor = 'blue';
		resultFb.innerHTML = 'Làm tốt lắm, khá.';
		correctAns.innerHTML = 'Số câu đúng: ' + ca;
	} else if (percentage >= 50) {
		RColor = 'orange';
		resultFb.innerHTML = 'Bạn đã đạt rồi! đạt được trung bình.';
		correctAns.innerHTML = 'Số câu đúng: ' + ca;
	} else {
		RColor = 'red';
		resultFb.innerHTML = 'Bạn đã nỗ lực rồi, hãy cố gắng hơn.';
		correctAns.innerHTML = 'Số câu đúng: ' + ca;
	}
